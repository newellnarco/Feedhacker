...
| 6 | **`CWS_PUBLISHER_ID` resolves now — and Google refuses it (FH-055)** | maintainer | **Answered 2026-09-15 by Release run #24**, after five releases of “empty” warnings. The variable IS reaching Actions: it resolved to `project-46303a79-fd20-4ed8-859`. But the first real call to `cancelSubmission` came back **HTTP 403 `PERMISSION_DENIED`** — *“Permission denied on resource 'publishers/project-46303a79-…/items/kccajfog…' (or it might not exist)”*. That value looks like a **Google Cloud project id**, not a Chrome Web Store **publisher** id. **Maintainer action:** Dashboard → Publisher → Settings shows the publisher id; replace the variable with that. **Nothing waits on this** — the slot has been free for every upload since 0.4.7, so the step has never been needed. Removing it entirely remains a reasonable alternative. A second, separate fault is **ours**: the step prints “HTTP 403 … normal when nothing is in review”, which is false and is the FH-048 false-green class surviving in the non-200 branch. **UPDATE 2026-09-16 (run #27, third occurrence):** identical 403 on the same resource path — reproducible, not transient. **Our half of the fault is now FIXED AND PROVEN against the real API:** the step printed `cancelSubmission was REFUSED (HTTP 403). This is NOT 'nothing in review'…` instead of the old “normal when nothing is in review”. What remains is purely the **wrong id**, which is the maintainer's to replace (Dashboard → Publisher → Settings) or to delete the variable outright. It has still never mattered — the slot has been free for every upload since 0.4.7 — but it **will** matter the first time a release is cut while a version is in review, which is exactly the state 0.8.0 is in right now. |
| 8 | **Confirm 0.8.0 actually publishes on the store** | next session | **Uploaded and submitted 2026-09-16 20:01:33 UTC** (run #27) on the maintainer's explicit “push 8.0 to the store”. The log's `Publish successful` means **submitted for Google review, NOT approved** — so this is not done yet. **Check the store listing**, not the inbox: `https://chromewebstore.google.com/detail/feedhacker/kccajfoghkplakndamlohpepopdpelkb`. It is Live only when it reads *Version 0.8.0*. Prior versions took ~14–30 minutes. Until then the **submission slot is BLOCKED** — a further upload would fail `ITEM_NOT_UPDATABLE`, and `cancelSubmission` still cannot free it (item 6). So **do not cut another store release until 0.8.0 clears.** When confirmed, mark the 0.8.0 store column **Live** in `RELEASES.md` with the date read off the listing, and update §2 here. |
...
| **Submission slot** | **BLOCKED** — 0.8.0 submitted 2026-09-16 20:01:33 UTC and pending review. A further upload would fail `ITEM_NOT_UPDATABLE`, and `cancelSubmission` cannot free it (§1 item 6). **Do not cut another store release until 0.8.0 clears.** |
