...
| 6 | **`CWS_PUBLISHER_ID` was replaced by the maintainer 2026-09-16 — UNVERIFIED, and it ARMS the cancel step** | next session to verify | History: the old value `project-46303a79-fd20-4ed8-859` (a Google **Cloud project** id, not a Chrome Web Store **publisher** id) drew an identical **HTTP 403 `PERMISSION_DENIED`** on releases #24, #25 and #27 — deterministic, not transient. **Our half is fixed and proven:** run #27 was the first to execute the corrected message and it reported the refusal honestly instead of calling it “normal when nothing is in review”. **The maintainer has now updated the repo Action variable.** This is **unverified from here**: the REST endpoint exists and the session has a `GITHUB_TOKEN`, but this environment's agent proxy refuses Actions paths (`403 “Access to this GitHub Actions path is not permitted through this proxy”`, tested both the single-variable and list endpoints), and the GitHub MCP server has no variables tool — a deliberate boundary, not something to work around. The job prints the value in plain text, so **the next release log is the first place to confirm it**: read the `CWS_PUBLISHER_ID:` line in the `webstore` job, and expect the cancel step to report 200/204 or 400/404 rather than 403. ⚠️ **This fix is not purely an improvement — it arms a blind destructive call.** `cancelSubmission` runs unconditionally before every store upload, against whatever is pending. While it was refused it was a guaranteed no-op. Now it can actually cancel. See §1 item 8 and `best_practices.md` §60. |
| 8 | **Confirm 0.8.0 publishes — and do NOT dispatch a store release until it does** | next session | **Uploaded and submitted 2026-09-16 20:01:33 UTC** (run #27). `Publish successful` means **submitted for Google review, NOT approved**, so this is not done. **Check the store listing**, never the inbox: `https://chromewebstore.google.com/detail/feedhacker/kccajfoghkplakndamlohpepopdpelkb`. It is Live only when it reads *Version 0.8.0*. (`WebFetch` caches 15 min per URL — vary the URL, e.g. `?hl=en-GB`, to force a fresh read, and say which you used.) 0.6.0 cleared in ~14 min, 0.7.0 in ~30. As of 20:37 UTC it still read 0.7.0. ⚠️ **The reason not to dispatch has INVERTED, so re-read it.** Before 2026-09-16 a store release cut now would have failed safely with `ITEM_NOT_UPDATABLE`. With `CWS_PUBLISHER_ID` replaced (item 6), the blind cancel step will now most likely **succeed** — withdrawing the in-review 0.8.0 and returning it to draft. The **same-version re-dispatch used earlier today is therefore no longer harmless.** Do not run the Release workflow with `store: true` until 0.8.0 is confirmed Live. When confirmed: mark the 0.8.0 store column **Live** in `RELEASES.md` with the date read off the listing, set §2 latest-on-the-store to 0.8.0 and the slot to OPEN, and close this item. |
...
| **Submission slot** | **BLOCKED** — 0.8.0 submitted 2026-09-16 20:01:33 UTC, pending review. ⚠️ **Do not cut another store release until it clears — and note the reason changed on 2026-09-16.** With the publisher id replaced, the blind `cancelSubmission` will now likely **succeed** and silently withdraw 0.8.0 from review, where it used to fail safely with `ITEM_NOT_UPDATABLE` (§1 items 6 and 8, `best_practices.md` §60). |
