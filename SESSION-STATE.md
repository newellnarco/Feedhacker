...
|---|---|---|---|
| 1 | **Verify the AI-slop fix on a real feed — and the export must be FRESH** | maintainer | FH-049. **A third export arrived 2026-09-15 and was again stale.** Its `version` field read `0.7.0`, but that is the manifest version *at export time* — the `log` entries all date from **2026-09-02 23:04–23:15**, and are the **original pre-fix numbers**: 300 decisions, **13 distinct posts**, 23.1 per post, one post ×42. Not evidence about any shipped fix. **Why it is stale, and this is the useful part:** `push()` keeps the NEWEST 300, so a log frozen at 2026-09-02 means **the scorer has not judged a single post in 13 days** — because **solo mode short-circuits before the scorer is consulted** (FH-051), and solo has been on the whole time. **So the measurement is impossible while solo is on.** Correct order: (1) **Reset AI-slop learning** — on 0.6.0+ this genuinely clears the log, training, observations and the self-tuned threshold (FH-054); (2) **turn solo OFF**; (3) browse a normal feed; (4) **Export log (JSON)**. Check the newest `log[].iso` is today's date before sending. **Decisions should ≈ distinct posts.** |
...
| **Dev cycle** | **0.8.0 open** — carries the FH-055 reporting fix (CI only, no user-visible change). **Do not release without an explicit “ship”** |
