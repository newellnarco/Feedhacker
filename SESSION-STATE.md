...
|---|---|---|---|
| 1 | **Three store/landing screenshots still show Solo — blocks the 0.8.0 STORE submission** | maintainer | The copy is updated everywhere, but the *images* are not: `store/screenshot-1-mixer.png` and `docs/carousel/carousel-2-mixer.png` show the popup with **M and S** buttons, and `docs/carousel/carousel-5-transparent.png` shows the options Actions card with the **Solo** row. **There is no screenshot generator in the repo** — the 2026-08-21 set was captured ad hoc from the built extension via the `test/system` harness and then composed in the brand style, so regenerating them is real work, not an edit. **This does not block the GitHub release**, only the store listing: shipping 0.8.0 to the store with these images would show users a control that no longer exists. Same class as FH-033 — listing assets and the uploaded package are independent publish channels. `store/screenshot-2-detection.png`, `-3-stub.png` and carousel 1/3/4 show the feed and stubs and are unaffected. |
...
| | |
|---|---|
| **Latest version** | **0.7.0** — released on GitHub 2026-09-15 (tag `v0.7.0` @ `dabdfc1`) and **LIVE on the store**, published 2026-09-15 19:54:24 UTC (~30 min after upload) |
| **Previous** | 0.6.0 — published 2026-09-15 18:24:40 UTC (~14 min after upload) |
| **Dev cycle** | **0.8.0 open** — solo mode removed (**FH-057**, user-visible) plus the FH-055 reporting fix (CI only). **Do not release without an explicit “ship”** |
