...
| 6 | **`CWS_PUBLISHER_ID` was replaced by the maintainer 2026-09-16 — UNVERIFIED, and it ARMS the cancel step** | next session to verify | History: the old value `project-46303a79-fd20-4ed8-859` (a Google **Cloud project** id, not a Chrome Web Store **publisher** id) drew an identical **HTTP 403 `PERMISSION_DENIED`** on releases #24, #25 and #27 — deterministic, not transient. **Our half is fixed and proven:** run #27 was the first to execute the corrected message and it reported the refusal honestly instead of calling it “normal when nothing is in review”. **The maintainer has now updated the repo Action variable.** Nothing in a session can read an Actions variable, so this is **unverified** — the job prints it in plain text, so **the next release log is the first place to confirm it**: read the `CWS_PUBLISHER_ID:` line in the `webstore` job, and expect the cancel step to report 200/204 or 400/404 rather than 403. ⚠️ **This fix is not purely an improvement — it arms a blind destructive call.** `cancelSubmission` runs unconditionally before every store upload, against whatever is pending. While it was refused it was a guaranteed no-op. Now it can actually cancel. See §1 item 8 and `best_practices.md` §60. |
...
- **It is UNVERIFIED and cannot be verified from a session** — nothing here can read an Actions
  variable. The `webstore` job echoes it in plain text, so the **next release log** is the first
  place to confirm it: expect a 200/204 or 400/404 from the cancel step instead of a 403.
