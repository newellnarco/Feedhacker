...
| NS-1 | Custom ast-grep rules for FeedHacker's hot bug-shapes | Was: add `.coderabbit/ast-grep/*.yml` rules so recurring classes are caught mechanically — (a) a `chrome.*` call not preceded by a liveness/`contextAlive` guard (best_practices §6); (b) assigning user/post text to `innerHTML`/`insertAdjacentHTML` instead of `textContent` (§12); (c) a second `chrome.storage.*.set` on a key that already has a writer (§7). | ❌ Cancelled — CodeRabbit no longer covers this repo at all (`REVIEWERS_STATUS.md`), so there is nothing to host these rules. If we still want them enforced mechanically, re-file as a plain CI lint step (ast-grep or eslint) — it belongs in CI, which is the only gate. | ⬜ | ⬜ |
