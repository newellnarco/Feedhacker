...
| Feed / DOM layer | `feed.ts` | `feed`, `grouping`, `author-identity`, **`post-identity`**, **`furniture`**, **`mark-slop`**, **`newsletter`** (integration) + `extension` (system) | content re-apply, stub UX, grouping, who Mute keys on, **how often a post is judged** (§46–48), and **what counts as a post at all** — LinkedIn's own modules wear the same marker (FH-053). Also **which direction a correction can be clicked from**: a label with no surface is a bias, not a missing feature (FH-062, §68), and the evidence a correction needs must outlive a re-render in the ledger rather than being recomputed — recomputing it is FH-049's flood again | ❌ run row + system |
| Content glue (storage/msg) | `content.ts` | `content-boot`, `content-teardown`, `slop-verdict-queue`, **`authors-reapply-loop`** (integration) + `extension` (system) | everything on the page; storage read-modify-write ordering (§7) — and **which storage changes are allowed to re-apply the feed**: `onChanged` fires in the tab that made the write, so a handler reacting to its own write loops forever at the debounce interval (FH-060, §64). The system tier owns the "how often is a post judged" invariant, because only a real browser runs the debounced write, its delivery back into the same tab, and the observer together | ❌ run row + system |
...
| Authors (mute/allow) | `authors.ts` | `authors` (unit), `author-identity`, **`authors-reapply-loop`** (integration), `extension` (system) | author memory — and the DOM-side identity that feeds it (§36). The store mixes **rules** (mute/allow, which change filtering) with **tallies** (counts, which change a chart only); `rulesKey`/`sameRules` keep them apart and the unit row pins that they cannot be confused | ❌ run row + system |
...
| **One-time migrations** | `background.ts` `onInstalled`, `filters.ts` `SLOP_LOCAL_KEYS`/`SLOP_SYNC_KEYS` | `factory-reset` (unit), `install-upgrade` (integration), `options.system` (system) | **stored user state a code fix cannot repair** — a destructive repair that runs twice, or that has never been banked on a fresh install, costs the user a model they trained (§67). The unit row pins that both callers clear ONE shared list and that the migration ledger is exempt from factory reset **by name**; the integration row pins run-once, clear-only-the-AI, and ledger-written-after-the-clear; the system row is the only tier that can prove `importScripts("filters.js")` works in a real MV3 service worker — a failed import makes the migration silently self-skip | ❌ run row + system |
...
| Release / store publish | `.github/workflows/release.yml` | `release-cancel`, **`release-gates`** (unit — the two publish gates evaluated under all four dispatch/tag situations, plus the MSI's `-acceptEula wix7` switch and its best-effort status) | the store-publish path — runs against the maintainer's real credentials and can't be exercised end-to-end from CI, so the shell is extracted from the YAML and executed against a stub `curl`, and the `webstore` job's gate is evaluated rather than matched. A store upload auto-updates **every existing install**, so “did this run publish there?” is not a question to answer by reading | ❌ run row |
| **Store listing text & graphics** | `store/listing.md`, `store/release-notes.md`, `scripts/store-notes.mjs`, `scripts/store-shots.mjs` | `store-notes` (unit) | **claims nothing else checks.** A hand-made PNG that stops being true fails no test, which is how four of ten listing images came to advertise a removed feature, a stale version, a retired extension id and an impossible group row (FH-065, §70). The unit row pins the text half — the shipping version has a store note, notes are newest-first, nothing mentions solo, every filter in `Filters.FILTERS` is named in the description, and no note parses to an empty body. The **images** are guarded by construction instead: `npm run store:shots` regenerates them from the extracted **store zip** (never `dist/feedhacker/`, which is the sideload build) and refuses a package carrying a `key` or `nativeMessaging` | ❌ run row; regenerate images on any UI change |
...
| In-product help | `options.html`, `popup.html` help text | **`help-coverage`** (unit) | what the user is told a control does | ❌ run row — and it runs whenever a **control** changes, since it reads `data-fh-act` out of `feed.ts` |
