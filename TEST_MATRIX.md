...
| Feed / DOM layer | `feed.ts` | `feed`, `grouping`, `author-identity`, **`post-identity`** (integration) + `extension` (system) | content re-apply, stub UX, grouping, who Mute keys on, and **how often a post is judged** (§46–48) | ❌ run row + system |
...
| **Destructive controls** | `options.ts` reset paths, `LOCAL_KEYS` | `factory-reset` (unit), `options.system` (system) | what a reset actually clears — never "presentational": it deletes user data, so it needs the real button against real storage (§52) | ❌ run row + system |
...
| **Solo / mute modes** | `feed.ts` solo branch, `filters.ts` `labelFor` | `solo-mode` (integration), `filters` (unit) | whether a hiding mode explains itself and can be exited from the feed (§50) | ❌ run row |
...
| Release / store publish | `.github/workflows/release.yml` | `release-cancel` (unit) | the store-publish path — runs against the maintainer's real credentials and can't be exercised end-to-end from CI, so the shell is extracted from the YAML and executed against a stub `curl` | ❌ run row |
