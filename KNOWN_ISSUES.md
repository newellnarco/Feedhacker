...
|---|---|---|---|---|---|
| ✅ | CodeRabbit never reviewed our PRs ("Review skipped — Draft detected") | We open PRs as drafts; CodeRabbit skips drafts by default and had no repo config | self (this session) | Add `.coderabbit.yaml` with `auto_review.drafts: true` (+ knowledge_base wiring) | n/a (config); verified by CodeRabbit reviewing the next draft PR |
