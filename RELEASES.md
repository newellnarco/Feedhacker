...
| 0.6.0 | ⏳ Dispatching (2026-09-15) | ⏳ Not yet uploaded | — | **Shipping on the maintainer's explicit "ship it".** Slot confirmed **OPEN** before dispatch by reading the store listing (0.5.0, updated 2026-09-08) rather than the inbox — see the 0.5.0 row for why the inbox is not evidence for this item. Contents: **FH-052** the selector-break heartbeat had been disarmed since LinkedIn's redesign (`contentCount()` read 0 against 8 and 42 real posts, so the alarm could never fire); **FH-053** LinkedIn's own feed modules were scanned, author-attributed and hidden as posts; **FH-054** "Reset AI-slop learning" cleared only the weights, so the model rebuilt itself from the surviving training data, observations and self-tuned threshold. Plus a release note for the ≤0.4.5 Windows sideload re-install (FH-042), and tests pinning the fresh-install / upgrade / factory-reset behaviour that was already correct. 258 unit+integration, 24 system. **This row records only what is true before dispatch — the workflow and store outcomes are filled in from the logs and the listing afterwards, never predicted (§44/§45).** |
