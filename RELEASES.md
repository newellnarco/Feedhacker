...
| 0.10.0 | ✅ Released (2026-09-18) — tag `v0.10.0` @ `eb67b23`, GitHub Release **published 14:28:35 UTC** | ✅ **Live** — confirmed 2026-09-18 **15:13 UTC** from the listing (`?hl=en-ZA`: *"Version 0.10.0, Updated 18 September 2026"*) | 2026-09-18 | **Shipped on the maintainer's explicit "ship it".** Slot confirmed **OPEN** first by reading the **store listing** at `?hl=en-IE` — *"Version 0.9.1, Updated 18 September 2026"* — a different URL from the earlier read that day, because `WebFetch` caches 15 minutes per URL. **Release run #29** (`workflow_dispatch` from `main`, `publish: true`, `store: true`, **`cancel_pending: false`**): `build` ✅, `webstore` ✅, `release` ✅, best-effort `msi` ❌ on the WiX gate as every release (blocks nothing, and is why the Release carries four assets rather than five). Assets, all uploaded 14:28:34: `feedhacker-0.10.0.zip` (127,983 B), `-store.zip` (127,000 B), `-win.zip` (140,520 B), `-store-submission.zip` (1,031,370 B). `webstore` job log, verbatim: `Uploading feedhacker-0.10.0-store.zip…` **14:28:18** / `Publishing…` / `Publish successful` **14:28:27** — that means **submitted for Google review, NOT approved**. ⚠️ **Do not mark this Live until the store listing itself shows 0.10.0.** `CWS_AUTO_PUBLISH: true`, so it publishes itself on approval. **The cancel step was SKIPPED** by `cancel_pending: false` — nothing was in review, and the workflow's own warning is that a refused cancel leaves the slot state *unknown* rather than proving it empty. **New, and only visible in a run log: `CWS_PUBLISHER_ID` is now `099a34bd-2c03-4251-a3e0-a4e39f521dad`** — a bare UUID, i.e. the shape of a real Chrome Web Store publisher id, where runs #24–#25 printed the Google Cloud project id `project-46303a79-fd20-4ed8-859` that caused FH-055. The variable has been corrected. This run does **not** prove `cancelSubmission` now works, because the cancel step did not execute — that needs a run where something is actually in review. Contents: **FH-067** (grouping buckets a run by filter instead of demanding three adjacent posts of one kind — the maintainer's screenshot showed seven stubs with the setting ON) and **FH-066** (the in-product help for the shown-post AI-slop splat, Show anyway and Hide again, plus the unit test that enumerates controls from `data-fh-act` and fails when one is undocumented). 186 unit + 137 integration + 37 system.  **CONFIRMED LIVE 2026-09-18 15:13 UTC** by reading the listing at `?hl=en-ZA` — it shows *"Version 0.10.0, Updated 18 September 2026"*. Uploaded 14:28:27, so published **within ~45 minutes**; the exact publish instant is not knowable from the listing and is deliberately not invented here. The slot is **OPEN** again. **Note on assets: this release has four, not five** — the `msi` job was still failing on `WIX7015` when it was cut. The fix (`-acceptEula wix7`, FH-068) landed after, so the MSI returns from the **next** release onward. |
| 0.9.1 | ⏳ **No GitHub tag or Release** — uploaded to the store by hand, so nothing was cut here (`dist/feedhacker-0.9.1-store.zip`) | ✅ **Live** — confirmed 2026-09-18 from the listing (`?hl=en-AU`: *"Version 0.9.1, Updated 18 September 2026"*) | 2026-09-18 | **Prepared for a MANUAL dashboard upload at the maintainer's request**, not via the Release workflow. Contents: **FH-063** (the interaction hold-off was armed from inside the click handler, so the first click in a while — the one a user notices — was unprotected; pointerdown and focusin now arm it, and a post whose own control has focus is left alone), **FH-064** (Insights renders the per-filter breakdown that `history[day].byId` has always stored and nothing ever displayed), and the **listing-graphics regeneration** below. **The image audit found more than the record claimed.** §1 item 1 named three images showing Solo; checking all ten found **four** wrong: `store/screenshot-1-mixer.png`, `docs/carousel/carousel-2-mixer.png` and `carousel-5-transparent.png` (Solo — and carousel-5 also showed **version 0.4.7** and the extension id of the **old** store item `bbfikimnhcdpgdebiipjbmagbklpbkde`), plus `carousel-3-group.png`, which showed a group row reading “Promoted ×1, AI Slop ×2, Hiring ×1” — a mixed row **FH-061 made impossible in 0.9.0**. Also found: the store description omitted **Company / brand posts**, one of the nine shipped filters, and `options.html` still carried a paragraph explaining Solo's removal. **All four images are regenerated from the BUILT STORE PACKAGE** by the new `npm run store:shots`, which extracts `dist/feedhacker-<v>-store.zip` rather than loading `dist/feedhacker/` — the unpacked tree is the *sideload* build, and screenshotting it had put “Permissions: storage, nativeMessaging” in a listing image directly beside the claim “only permission: storage”. 314 unit+integration, 36 system.  **PUBLISHED.** The maintainer uploaded the package and the graphics through the Developer Dashboard on 2026-09-18 and reported it published; **confirmed by reading the listing** at `?hl=en-AU` (a varied URL, since `WebFetch` caches 15 minutes per URL), which shows *"Version 0.9.1, Updated 18 September 2026"*. So this version reached users **without a tag, a GitHub Release or a workflow run** — the first one to do so, and the reason the GitHub column above is not a ✅. The submission slot is OPEN again. |
